...
class TriFusionApp(App):
    ...
    def orto_compare_groups(self):
        ...
        active_groups = ot.MultiGroupsLight(shelve_path=join(self.temp_dir,
                                                             "act_groups.db"))
    ...
    def orto_change_filters(self, gn_filter, sp_filter, group_name=None,
        ...
        self.active_group = self.ortho_groups.get_group(self.active_group.name)
    ...
    def load_group_files(self, group_files):
        ...
        if not self.ortho_groups:
            self.ortho_groups = ot.MultiGroupsLight(
                shelve_path=join(self.temp_dir, "groups.db"))

        for gf in group_files:
            self.ortho_groups.add_group(ot.GroupLight(gf))

        return [list(group_files), self.ortho_groups.filters]
    ...
    def load_groups(self, group_names, default_filters):
        """
        ...
        self.run_in_background(self.orto_update_filters, self.orthology_card,
                               [None, None, group_names, True], None, True)
    ...
    def orthology_card(self, group_name=None, bt=None):
        ...
        # Update active group
        self.active_group = self.ortho_groups.get_group(group_name)
        ...
        stats = self.active_group.basic_group_statistics()
        ...
        cards.taxa_txt = str(len(self.active_group.species_list))
        cards.group_name = self.active_group.name.split(sep)[-1]

        # Create gauge plots, if there are any filtered groups
        if self.active_group.species_threshold or \
                self.active_group.gene_threshold or \
                (self.active_group.species_threshold,
                 self.active_group.gene_threshold) == (0, 0):
