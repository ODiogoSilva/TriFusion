...
class Partitions ():
	""" The Partitions class is used to parse and work with partition files. Currently the supported input formats are
	RAxML partition files and Nexus chartset blocks. To initialize a class, only a file partition file is required. """
	...
	def _get_partitions(self, partitions_file):
		""" This function parses a file containing partitions. Supports partitions files similar to RAxML's and NEXUS
		charset blocks. The NEXUS file, however, must only contain the charset block. The model_nexus argument provides
		a namespace for the model variable in the nexus format, since this information is not present in the file.
		...
		if self.partition_format == "raxml":
			for line in part_file:
				...
				partition_storage.append((model, partition_name, partition_range))  # Format of partition storage: [
				# "str","str",["str","str"]]
			...
			for line in part_file:
				...
				partition_storage.append((self.model_nexus, partition_name, partition_range))  # Format of partition
				# storage: ["str", str", ["str","str"]]
			
		return partition_storage

	def write_to_file(self, output_format, output_file, model="LG"):
		""" Writes the Partitions object into an output file according to the output_format. The supported output
		formats are RAxML and Nexus. The model option is for the RAxML format """
		...
		if output_format == "raxml":
			...
			for part in self.partitions:
				...
				outfile_handle.write("%s, %s = %s\n" % (model, partition_name, partition_range))
	...
	def __init__(self, alignment_list, suffix="_zorro.out"):
		...
		def zorro2rax (self, alignment_list):
			...
			for alignment_file in alignment_list:
				zorro_file = alignment_file.split(".")[0]+self.suffix  # This assumes that the prefix of the
