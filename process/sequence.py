...
class Alignment (Base, MissingFilter):
	...
	def __init__(self, input_alignment, input_format=None, model_list=None, alignment_name=None, loci_ranges=None):
		""" The basic Alignment class requires only an alignment file and returns an Alignment object. In case the
		class is initialized with a dictionary object, the input_format, model_list, alignment_name and loci_ranges
		arguments can be used to provide complementary information for the class. However, if the class is not
		initialized with specific values for these arguments, they can be latter set using the _set_format and
		_set_model functions

		The loci_ranges argument is only relevant when an Alignment object is initialized from a concatenated data set,
		in which case it is relevant to incorporate this information in the object"""
		...
		# In case the class is initialized with a dictionary object
			...
			self.init_dicobj(input_alignment)  # Gets several attributes from the dictionary alignment
			...
			if loci_ranges is not None:
				self.loci_ranges = loci_ranges  # A list containing the ranges of the alignment, in case it's a
				# concatenation
	...
	def _set_model(self, model_list):
		""" Use this function to manually set the model associated with the Alignment object. Since this object
		supports concatenated alignments, the model specification must be in list format and the list size must be of
		the same size of the alignment partitions """

		self.model = model_list

	def _set_alignment(self, alignment_dict):
		""" This function can be used to set a new alignment dictionary to the Alignment object. This may be usefull
		when only the alignment dict of the object has to be modified through other objects/functions """
	...
	def init_dicobj(self, dictionary_obj):
		""" In case the class is initialized with a dictionary as input, this function will retrieve the same
		information as the read_alignment function would do  """
	...
	def read_alignment(self, input_alignment, alignment_format, size_check=True):
		""" The read_alignment method is run when the class is initialized to parse an alignment an set all the basic
		attributes of the class.

		The 'alignment' variable contains an ordered dictionary with the taxa names as keys and sequences as values
		The 'model' is an non essential variable that contains a string with a substitution model of the alignment.
		This only applies to Nexus input formats, as it is the only supported format that contains such information
		...
		self.model = []  # Only applies for nexus format. It stores any potential substitution model at the end of
		# the file
		...
		# PARSING NEXUS FORMAT
			...
			for line in file_handle:
				if line.strip().lower() == "matrix" and counter == 0:  # Skips the nexus header
					...
					taxa = self.rm_illegal(taxa)
					if taxa in self.alignment:  # This accommodates for the interleave format
		...
		if len(list(self.alignment)) != len(set(list(self.alignment))):
			...
			self.log_progression.write("WARNING: Duplicated taxa have been found in file %s (%s). Please correct this "
								"problem and re-run the program\n" % (input_alignment, ", ".join(taxa)))
			raise SystemExit
	...
	def remove_taxa(self, taxa_list_file, mode="remove"):
		""" Removes specified taxa from the alignment. As taxa_list, this method supports a python list or an input
			csv file with a single column containing the unwanted species in separate lines.
	...
	def collapse(self, write_haplotypes=True, haplotypes_file=None):
		""" Collapses equal sequences into haplotypes. This method changes the alignment variable and only returns a
		dictionary with the correspondence between the haplotypes and the original taxa names  """
		...
		if write_haplotypes is True:
			# If no output file for the haplotype correspondence is provided, use the input alignment name as reference
	...
	def reverse_concatenate(self, partition_obj):
		""" This function divides a concatenated file according previously defined partitions and returns an
	...
	def code_gaps(self):
		""" This method codes gaps present in the alignment in binary format, according to the method of Simmons and
		Ochoterena (2000), to be read by phylogenetic programs such as MrBayes. The resultant alignment, however, can
		 only be output in the Nexus format """

		def gap_listing(sequence, gap_symbol="-"):
			""" Function that parses a sequence string and returns the position of indel events. The returned list is
			composed of tuples with the span of each indel """
			gap = "%s+" % gap_symbol
			span_regex = ""
		...
		def gap_binary_generator(sequence, gap_list):
			...
			for cur_gap in gap_list:
				...
				if sequence[cur_gap_start:cur_gap_end] == "-" * (cur_gap_end - cur_gap_start) and sequence[
							cur_gap_start - 1] != "-" and sequence[cur_gap_end] != "-":
					sequence += "1"
	...
	def filter_missing_data(self, gap_threshold, missing_threshold):
		...
		# When the class is initialized, it performs the basic filtering operations based on the provided thresholds
		alignment_filter = MissingFilter(self.alignment, gap_threshold=gap_threshold,
										missing_threshold=missing_threshold, gap_symbol="-",
	...
	def write_to_file(self, output_format, output_file, new_alignment=None, seq_space_nex=40, seq_space_phy=30,
		...
		The choice of this modes is determined by the presence or absence of the loci_range attribute of the object.
		...
		if self.restriction_range is not None:
			if output_format != ["nexus"]:
				self.log_progression.write("OutputFormatError: Alignments with gaps coded can only be written in Nexus"
											" format")
				return 0
		else:
			pass
		...
		if "mcmctree" in output_format:
			...
			if self.loci_ranges is not None:
				for element in self.loci_ranges:
					...
					for taxon, seq in self.alignment.items():
						out_file.write("%s  %s\n" % (taxon[:cut_space_phy].ljust(seq_space_phy), seq[(int(
							partition_range[0]) - 1):(int(partition_range[1]) - 1)].upper()))
			else:
		...
		if "nexus" in output_format:
			...
			if form == "interleave":
				...
				for i in range(90, self.locus_length, 90):
					...
					for key, seq in alignment.items():
						out_file.write("%s %s\n" % (key[:cut_space_nex].ljust(seq_space_nex), seq[i:self
							.locus_length].upper()))
					else:
						out_file.write("\n")
				out_file.write(";\n\tend;")
			...
			if outgroup_list is not None:
				...
				compliant_outgroups = [taxon for taxon in outgroup_list if taxon in self.iter_sequences()]  # This
				# assures that only the outgroups present in the current file are written
...
class AlignmentList (Alignment, Base, MissingFilter):
	""" At the most basic instance, this class contains a list of Alignment objects upon which several methods can be
	applied. It only requires either a list of alignment files or .
	...
	def __init__(self, alignment_list, model_list=None, name_list=None, verbose=True, input_format=None):
		...
		if type(alignment_list[0]) is str:
			...
			# Setting class flag so that the object is aware that is a result of reverse concatenation and not an
			# actual list of alignments
			...
			for alignment, model, name in zip(alignment_list, model_list, name_list):
				...
				# In some cases, like reverse concatenating, the input format must be manually set
				if input_format is not None:
	...
	def concatenate(self, progress_stat=True):
		...
		for alignment_object in self.alignment_object_list:
			...
			self.loci_range.append((alignment_object.input_alignment.split(".")[0], "%s-%s" % (sum(self.loci_lengths)
	...
	def remove_taxa(self, taxa_list, verbose=False, mode="remove"):
		""" Wrapper of the remove_taxa method of the Alignment object for multiple alignments. It current supports two modes:
	...
	def select_by_taxa(self, taxa_list, mode="strict", verbose=True):
		...
		# taxa_list may be a file name (string) or a list containing the name of the taxa. If taxa_list is a file name,
		# this code will parse the csv file and return a list of the taxa. Otherwise, the taxa_list variable remains
		# the same.
		try:
